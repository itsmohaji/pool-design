...
export default function Experience() {
  ...
  return <div className="experience" ref={root}>
    ...
    <header className="topbar"><div className="brand"><i>LH</i><div><b>LIJBAILAT VILLA</b><small>POOL TERRACE EXPERIENCE</small></div></div><div className="topActions"><button onClick={()=>notify("Sound ambience is ready for final licensed audio")}>◌</button><button onClick={()=>document.documentElement.requestFullscreen?.()}>⛶</button><button className="design" onClick={()=>state.set({panel:true})}>Design Studio <span>＋</span></button></div></header>
    <nav className="views"><small>CURATED VIEWS</small>{[["overview","Aerial"],["pool","Pool edge"],["lounge","Lounge"]].map(([id,label])=><button className={camera===id?"active":""} onClick={()=>setCamera(id)} key={id}><span>{label}</span></button>)}</nav>
    <button className="minimap" onClick={()=>setCamera("pool")} aria-label="Travel to pool viewpoint"><span className="poolMark"></span><i></i><b>POOL PLAN</b></button>
